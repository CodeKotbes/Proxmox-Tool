...
def api_add_script():
    ...
    new_id = database.add_script(d.get("name"), d.get("description", ""), d.get("content"))
...
def api_clone_vm():
    ...
    if not s_ids and d.get("script_id"):
        s_ids = [d.get("script_id")]

    # Wir lesen 'data_disk_gb' aus dem Request (Default: 3 GB)
    data_size = d.get("data_disk_gb", 3)
    ...
    threading.Thread(target=tasks.run_clone_task, args=(
        ...
        s_ids,
        data_size # <--- Neuer Parameter am Ende
    )).start()
    return jsonify({"status": "started", "job_id": jid}), 202
...
if __name__ == "__main__":
    database.init_db() # DB sicherstellen
