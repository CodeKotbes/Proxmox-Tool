...
def get_proxmox_api():
    try:
        return ProxmoxAPI(PVE_HOST, port=PVE_PORT, user=PVE_USER, password=PVE_PASSWORD, verify_ssl=False)
    except Exception as e:
        print(f"Connection failed: {e}")
        return None

def delete_vm(node, vmid):
    """Versucht eine VM zu löschen, fängt Fehler ab falls sie nicht existiert."""
    try:
        # Versuche erst zu stoppen
        try:
            node.qemu(vmid).status.stop.post()
            time.sleep(8) # Warten bis Stop durch ist
        except:
            pass 
        # Dann löschen
...
def run_create_template_task(job_id, vmid, name, image_path, storage):
    ...
    try:
        ...
        node.qemu.post(vmid=vmid, name=name, memory=2048, net0=f"virtio,bridge={NAT_BRIDGE}", scsihw="virtio-scsi-pci", ostype="l26", agent=1)
        vm_created = True
        time.sleep(3)
...
# Updated Clone Task with start_order
def run_clone_task(job_id, template_vmid, new_vm_name, cores, memory, disk_size, start_order, tag, user, password, ssh_key, ip_cidr, gateway, storage):
    ...
    final_user = user if user else DEFAULT_USER
    final_pass = password if password else DEFAULT_PASS
    ...
    try:
        ...
        config_payload = {
            ...
            "ipconfig0": ip_config,
            "tags": tag,
            "net0": f"virtio,bridge={NAT_BRIDGE}"
        }
        if encoded_key:
    ...
    try:
        ...
        for vm in vms:
            ...
            try:
                ...
                copy_user = old_conf.get('ciuser', DEFAULT_USER)
                ...
                copy_startup = old_conf.get('startup', None) # Startreihenfolge sichern
                
                # --- STORAGE & DISK SIZE ERMITTELN ---
                old_disk_size_gb = None
                target_storage_id = None
                
                disk_config = old_conf.get('scsi0', '')
                # Beispiel string: 'local-zfs:vm-105-disk-0,size=32G'
                
                if ':' in disk_config:
                    # Alles vor dem ersten Doppelpunkt ist der Storage
                ...
                if target_storage_id:
                    ...
                    clone_params['full'] = 1 # Full clone ist pflicht bei storage wechsel/wahl
                
                node.qemu(new_template_id).clone.post(**clone_params)
                time.sleep(15)
                
                # 4. RESIZE (Wiederherstellen)
                if old_disk_size_gb:
                    try:
                ...
                # Startreihenfolge wiederherstellen
                if copy_startup:
...
def api_create_template():
    ...
    threading.Thread(target=run_create_template_task, args=(jid, d.get("vmid"), d.get("name"), d.get("image"), d.get("storage"))).start()
...
def api_clone_vm():
    ...
    threading.Thread(target=run_clone_task, args=(
        jid, 
        d.get("template_vmid"), 
        d.get("vm_name"), 
        d.get("cores"), 
        d.get("memory"), 
        d.get("disk_size"), 
        d.get("start_order"), # <-- NEU: Start Order
        d.get("tag"), 
        d.get("ci_user"), 
        d.get("ci_password"), 
        d.get("ssh_key"),
...
def api_redeploy_vms():
    ...
    threading.Thread(target=run_redeploy_task, args=(
        jid, 
        d.get("old_tag"), 
